...
ui <- fluidPage(
  ...
  div(style="display: inline-block;", h6(style="display: inline-block;", "By "), a(href="https://www.linkedin.com/in/hernangonzalezrossi/", target="_blank", "Hernan")),
  br(),
  ...
  tabsetPanel(
    tabPanel('Palabras en la pagina',
             sidebarLayout(
               ...
               mainPanel(
                 fluidRow(column(12, plotOutput('WC', height = "600px"))),
                 width = 9,
                 height = 12
               )
             )
...
server <- function(input, output, session) {
  ...
  output$WC <- renderPlot({
    ...
    all_text %>%
      ...
      count(word, sort=T) %>%
      with(wordcloud(word, n, scale=c(2,0.5), max.words = input$bins, random.order=FALSE, color = brewer.pal(8, "Dark2")))
